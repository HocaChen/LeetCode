...
class Solution:
    def distributeCandies(self, candyType):
        ...
        diff_candy_type = 0
        pre_data = 0
        len_candy = len(candyType)/2
        candyType.sort()
        for idx, x in enumerate(candyType):
            if idx == 0 or x != pre_data:
                pre_data = x
                # candyType[diff_candy_type] = x
                diff_candy_type += 1
        if len_candy < diff_candy_type:
            return len_candy
        else:
            return diff_candy_type

# candyType = [1,1,2,2,3,3]
# candyType = [1,1,2,3]
# candyType = [6,6,6,6]

candyType = [100000,0,100000,0,100000,0,100000,0,100000,0,100000,0]

k = Solution.distributeCandies(candyType, candyType)
print (k)
